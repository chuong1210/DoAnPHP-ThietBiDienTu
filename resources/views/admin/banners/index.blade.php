...
@section('content')
    <div class="container-fluid"
        ...
        <!-- Add Button -->
        <a href="{{ route('admin.banners.create') }}" class="btn btn-lg mb-4"
                        ...
                        <tbody>
                            @forelse ($banners as $banner)
                                <tr style="border-bottom: 1px solid #CBD5E1; transition: background-color 0.2s;">
                                    ...
                                    <td style="color: #1E293B; padding: 16px;">
                                        @if ($banner->link)
                                            <a href="{{ $banner->link }}" target="_blank"
                                                style="color: #0066FF; text-decoration: none; font-size: 13px;">{{ \Illuminate\Support\Str::limit($banner->link, 20) }}</a>
                                        @else
                                            -
                                        @endif
                                    </td>
                                    ...
                                    <td style="padding: 16px;">
                                        <span class="badge"
                                            style="background-color: {{ $banner->is_active ? '#00B4D8' : '#CBD5E1' }}; color: white; padding: 6px 12px; border-radius: 4px;">
                                            {{ $banner->is_active ? 'Hiển thị' : 'Ẩn' }}
                                        </span>
                                    </td>
                                    <td style="padding: 16px; display: flex; gap: 8px;">
                                        <a href="{{ route('admin.banners.edit', $banner->id) }}" class="btn btn-sm"
