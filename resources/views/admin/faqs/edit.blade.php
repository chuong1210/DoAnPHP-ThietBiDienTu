            ...
            <form action="{{ route('admin.faqs.update', $faq) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="question" class="form-control @error('question') is-invalid @enderror"
                           value="{{ old('question', $faq->question) }}" style="border-color: #F0D9DE;" required>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="answer" rows="6" class="form-control @error('answer') is-invalid @enderror"
                              style="border-color: #F0D9DE;" required>{{ old('answer', $faq->answer) }}</textarea>
                    ...
                    <select name="category" id="category" class="form-select select2-category" style="width: 100%;" required>
                        ...
                        @foreach($cats as $value => $cat)
                            <option value="{{ $value }}" data-icon="{{ $cat['icon'] }}"
                                    {{ old('category', $faq->category) == $value ? 'selected' : '' }}>
                                {{ $cat['name'] }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3 form-check">
                    <input type="checkbox" name="is_active" value="1" class="form-check-input"
                           {{ old('is_active', $faq->is_active) ? 'checked' : '' }} style="accent-color: #FF3B3F;">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="sort_order" class="form-control"
                           value="{{ old('sort_order', $faq->sort_order) }}" min="0" style="border-color: #F0D9DE;">
                </div>
